...
__device__ void multiply_shared_mem_scalar(float pix[SY][SSX], float scalar) {
  auto block = cg::this_thread_block();
  const int cnt = SY * SX;
  const int num_blocks = (cnt + BX * BY - 1) / (BX * BY);
  for (int b = 0; b < num_blocks; ++b) {
...
__global__ void fusedssimCUDA_kernel(
  int H,
  int W,
  int CH,
  float C1,
...
__global__ void fusedssim_backwardCUDA_kernel(
  int H,
  int W,
  int CH,
  float C1,
...
)
{
  ...
  for (int i = 0; i < CH; ++i) {
    ...
    block.sync();
    multiply_shared_mem_scalar(buf2, grad_scale);
...
void fusedssim_forward(
    ...
    fusedssimCUDA_kernel<<<grid, block>>>(
        H, W, channels, C1, C2,
        rendered_image, gt_image, ssim_map,
        dm_dmu1, dm_dsigma1_sq, dm_dsigma12
        );

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("SSIM Forward Launch Failed: %s\n", cudaGetErrorString(err));
    }
...
void fusedssim_backward(
    ...
    fusedssim_backwardCUDA_kernel<<<grid, block>>>(
        H, W, channels, C1, C2,
        rendered_image, gt_image, grad_scale, ssim_grads,
        dm_dmu1, dm_dsigma1_sq, dm_dsigma12
    );
    
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("SSIM Backward Launch Failed: %s\n", cudaGetErrorString(err));
    }
}
